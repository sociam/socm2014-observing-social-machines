...
The problem of authentication is that of being able to verify the identity of entities, including users, within a distributed system. In a traditional system, a user would typically log in to each system explicitly to authenticate with it, typically first establishing a principal for each first.  However, in a distributed system, explicitly establishing principals on every system is inefficient, requiring the creation and maintance of $O(n^2)$ principals.  Distributed identity systems, such as OpenID \cite{recordon2006openid}, WebID\cite{huang2000webids}, or Persona \cite{koshutanski2007distributed}, discussed earlier, meanwhile provide a solution that uses a proof-of-identity mechanism relying on common third parties, which are typically well-known  \emph{distributed identity providers}. We have added OpenID support into the INDX reference implementation to allow users to prove their identities to any other INDX Atom, and support for other protocols is planned for the future.

The problems of synchronisation is that of supporting simultaneous collaborative editing and use of shared information items in a partially disconnected environment, such as when a node is powered off, or otherwise disconnected from a network (such as for mobile devices during flights).  Allowing such items to continue to be edited and used by users while nodes are disconnected means that changes must be reconciled when nodes are re-attached.  Methods have been devised to support user-intervention-free reconciliation such as \cite{sun1998operational}, and INDX currently takes a simple opportunistic approach to handle a majority of such cases without user intervention.

Similarly, the challenge of longitudinal storage, briefly mentioned earlier, pertains to protecting stored data against catastrophic loss (i.e., from hardware failure) in ways that can be managed effectively by the user.  A solution is to provide a system that automatically replicates important data across an arbitrary number of INDX Atoms, located on physically separate and potentially distant locations, thereby reducing the likelihood that failure affecting any such node will result in data loss.  INDX embodies the LOCKSS principle\cite{reich2001lockss} and supports automatic fails-over when atoms fail or go offline. 

Finally enabling the use of INDX Atoms to perform distributed web observatory-style analysis, described in some of the scenarios described earlier, requires consideration of 
